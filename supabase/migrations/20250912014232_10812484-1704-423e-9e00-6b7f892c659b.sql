-- Remove the foreign key constraint from delivery_addresses to public.users
-- since we're using Supabase Auth (auth.users) for user management

-- First, let's check if there's a foreign key constraint
DO $$
DECLARE
...
-- Add a comment to clarify that user_id references auth.users
