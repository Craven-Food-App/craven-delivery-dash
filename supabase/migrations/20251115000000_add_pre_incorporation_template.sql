...
<div class="section">
  ...
  <table class="table">
    <thead><tr><th>Director Name</th><th>Address</th><th>Email</th><th>Term Notes</th></tr></thead>
    <tbody>
...
<div class="section page-break">
  ...
  <p><strong>Section 1.</strong> Officers. The principal officers shall be a Chief Executive Officer, President, Chief Financial Officer, Secretary, and others as the Board appoints.</p>
  ...
  <p><strong>Section 3.</strong> Duties. (a) CEO — general supervision and control of business and affairs. (b) President — acts as chief operating officer. (c) CFO — chief accounting and financial officer. (d) Secretary — keeps minutes and records of the corporation.</p>
...
-- Add signature fields for 'officer' role
-- These fields will be positioned on the "Officer/Director Acceptance" page
-- We need to get the template_id first, then insert signature fields
DO $$
DECLARE
...
BEGIN
  ...
  IF template_id_val IS NOT NULL THEN
    ...
    -- Insert signature field for officer on the Officer/Director Acceptance page (page 2)
    -- Position: Bottom left area where the signature line appears
    INSERT INTO public.document_template_signature_fields (
      ...
      'signature',
      'officer',
      2, -- Page 2 (Officer/Director Acceptance section)
      15.0, -- x position: 15% from left
      75.0, -- y position: 75% from top (near bottom of page)
    ...
    RAISE NOTICE 'Added signature fields for pre_incorporation_consent template (template_id: %)', template_id_val;
  ELSE
    RAISE WARNING 'Template pre_incorporation_consent not found';
  END IF;
END $$;
