-- Grant universal access to craven@usa.com by bypassing RLS entirely
-- This ensures craven@usa.com can access everything without any restrictions

-- Create a function that checks if user is craven@usa.com and grants universal access
-- We'll use this in policies to bypass all checks for this user
...
BEGIN
  ...
  RETURN user_email = 'craven@usa.com';
END;
$$;
...
-- Now update ALL policies to check craven@usa.com FIRST before any other checks
-- This ensures universal access

-- user_roles: Universal access for craven@usa.com
...
-- board_members: Universal access
...
-- governance_board_resolutions: Universal access
...
-- board_resolution_votes: Universal access
...
-- corporate_officers: Universal access
...
-- executive_appointments: Universal access
...
-- governance_logs: Universal access
