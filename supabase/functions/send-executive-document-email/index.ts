...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!emailTemplate) {
      return new Response(
        JSON.stringify({ 
          error: "Email template not found in database. Please create an email template with template_key 'executive_document_email' or set it in template_usage for 'executive_document_email' context." 
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
