...
DROP POLICY IF EXISTS "Admins can update tablet shipping" ON restaurant_onboarding_progress;
DROP POLICY IF EXISTS "Admins can view onboarding progress" ON restaurant_onboarding_progress;
...
CREATE POLICY "Admins can update tablet shipping" ON restaurant_onboarding_progress
FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_profiles.user_id = auth.uid() 
...
CREATE POLICY "Admins can view onboarding progress" ON restaurant_onboarding_progress
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_profiles.user_id = auth.uid() 
...
CREATE POLICY "Admins can view restaurants" ON restaurants
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE user_profiles.user_id = auth.uid() 
...
CREATE POLICY "Admins can view user profiles" ON user_profiles
FOR SELECT USING (
  EXISTS (
